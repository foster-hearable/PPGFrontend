...
[Updated 2023] Some modifications were applied for Foster-Hearable.
*/
...
BlueJelly.prototype.setUUID = function(name, serviceUUID, characteristicUUID){
  ...
  this.hashUUID[name] = {'serviceUUID':serviceUUID, 'characteristicUUID':characteristicUUID, 'dataCharacteristic':null};
}
...
BlueJelly.prototype.requestDevice = function(uuid) {
  console.log('Execute : requestDevice : ' + uuid);
  var uuid_list = []
  for (let key in this.hashUUID){
    uuid_list.push(this.hashUUID[key].serviceUUID)
  }
  ...
  .then(device => {
    console.log('Execute : requestDevice : device found : ' + uuid);
...
BlueJelly.prototype.connectGATT = function(uuid) {
  ...
  if (this.bluetoothDevice.gatt.connected && this.hashUUID[uuid].dataCharacteristic) {
    if(this.hashUUID_lastConnected == uuid)
      return Promise.resolve();
  }
  this.hashUUID_lastConnected = uuid;

  console.log('Execute : connect : ' + uuid);
  ...
  .then(service => {
    console.log('Execute : getCharacteristic : ' + uuid);
    return service.getCharacteristic(this.hashUUID[uuid].characteristicUUID);
  })
  .then(characteristic => {
    this.hashUUID[uuid].dataCharacteristic = characteristic;
    this.hashUUID[uuid].dataCharacteristic.addEventListener('characteristicvaluechanged',this.dataChanged(this, uuid));
...
BlueJelly.prototype.read = function(uuid) {
  ...
  .then( () => {
    ...
    return this.hashUUID[uuid].dataCharacteristic.readValue();
  })
  .catch(error => {
    console.log('Error : ' + error);
    this.onError(error);
...
BlueJelly.prototype.write = function(uuid, array_value) {
  ...
  .then( () => {
    ...
    return this.hashUUID[uuid].dataCharacteristic.writeValue(data);
  })
  .then( () => {
...
BlueJelly.prototype.startNotify = function(uuid) {
  ...
  .then( () => {
    ...
    this.hashUUID[uuid].dataCharacteristic.startNotifications()
  })
  .then( () => {
...
BlueJelly.prototype.stopNotify = function(uuid){
  ...
  this.hashUUID[uuid].dataCharacteristic.stopNotifications()
})
  .then( () => {
...
BlueJelly.prototype.clear= function() {
   ...
   this.bluetoothDevice = null;
   this.hashUUID[uuid].dataCharacteristic = null;
   this.onClear();
}
